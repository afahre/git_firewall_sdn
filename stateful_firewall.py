        ...
        try:
            ...
            if(out_port != ofproto.OFPP_FLOOD) and (ethtype == ETH_TYPE_IP):
                ...
                if(ipo.proto == IPPROTO_ICMP):
                    ...
                    # Otherwise, PONG...!                 
                    elif (icmpob.type==ICMP_PONG) and self.icmp_conn_track.has_key(ipo.dst):
                                ...
                                if tmp[i][0] == ipo.src:
                                    ...
                                    if((xyz[1]=='PING') and (xyz[2]=='PONG')):
                                        ...
                                        self.flow.add_flow(datapath=datapath, actions=actions_default, priority=1001, in_port = in_port,
                                                       ...
                                                       ipv4_src = ipo.src, ipv4_dst= ipo.dst)
                                        self.icmp_conn_track = self.track.conn_track_dict(self.icmp_conn_track,ipo.src, ipo.dst, "PONG", "PING", xyz[3],1)
                                        self.logger.info("\n%s  ->  %s  action= PING  state= ESTABLISHED " % (ipo.dst,ipo.src))
